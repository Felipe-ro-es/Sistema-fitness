...
export default function PersonalRegister() {
  const { register } = usePersonalAuth();
  const navigate = useNavigate();
  const [form, setForm] = useState({ nome: "", email: "", senha: "", cref: "" });
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await register(form.nome, form.email, form.senha, form.cref);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-8">
          <h1 className="text-xl font-bold text-gray-900 mb-6">Cadastro de Personal Trainer</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-sm text-gray-600 mb-1.5 block">CREF</label>
              <input
                type="text"
                value={form.cref}
                onChange={e => sf("cref", e.target.value)}
                required
                className="w-full p-3 rounded-lg bg-gray-100 border border-gray-300 text-gray-900 focus:outline-none focus:border-[#ff6600] transition-colors"
          ...
          <p className="text-center text-sm text-gray-400 mt-6">
            Já tem conta?{" "}
            <Link to="/personal/login" className="text-[#ff6600] hover:underline font-medium">
              Entrar
            </Link>
          </p>
        </div>
